...
- **給与について**:
    ...
    - 応募可能な最下限の条件（例：学歴不問、高卒以上、年齢不問（記載なし）など）に対する最低保証給与が明記されているか厳密に確認してください。例えば、学歴不問なのに「大卒の給与」しかない場合や、年齢要件がないのに「20代前半の給与」しかない場合は指摘してください。さらに、「高卒の方は相談させていただきます」等の曖昧な表現で特定の条件に対する最低金額が明記されていない場合も、指摘対象としてください。
- **試用期間について**:
    - 試用期間が設定されており、かつ本採用時と労働条件（給与等）が異なる場合、1. 異なる条件の内容、2. それ以外の条件に変更はない旨、の両方が明記されているか。
    - 試用期間中の給与に固定残業代が含まれる場合、本採用時だけでなく試用期間用の固定残業代についても必須3項目（1.金額、2.時間数、3.超過分の追加支給）が全て明記されているか厳密に確認してください。片方でも漏れていれば指摘対象です。
- **改正職業安定法に基づく明示事項**:
